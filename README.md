...
여러분이 직접 치는 배포 명령은 세 줄입니다. 어려운 설정은 검증된 스크립트(`scripts/`)가 처리합니다.
...
| 회차 | 날짜 | 내용 | 완료 기준 (인증) |
| --- | --- | --- | --- |
| 1 | 8/31 (월) | 킥오프 — 여정 소개, 운영 규칙 합의 | 톡방에 "가져가고 싶은 것 한 가지" |
| 2 | 9/7 (월) | OpenStack 아키텍처 — 컴포넌트 6가지 ↔ AWS | 예습 질문 답하기 (세션 퀴즈) |
| 3 | 9/14 (월) | "버튼 뒤의 6단계" + 배포받은 VM에 SSH 접속·검증 | SSH 접속 + 검증 명령 스크린샷 |
| 4 | 9/21 (월) | 🌕 추석 전 라이트 — `setup.sh` 실행 | `READY ✅` 출력 스크린샷 (5회차 티켓) |
| 5 | 9/28 (월) | deploy 데이 — pull → deploy, `docker ps`로 구조 읽기 | `docker ps` 전부 Up |
| 6 | **10/6 (화)** | 인스턴스 생성 → Floating IP → SSH 🏁 + netns로 라우터 실체 보기 | `ssh cirros@<FIP>` 성공 화면 |
| 7 | 10/12 (월) | 버퍼 — 미완주자 구제 + 심화(로그로 6단계 추적) | 전원 완주 |
| 8 | 10/19 (월) | KPT 회고 + 발표 자료 제작 | KPT 제출 |
| 발표 | 10/24 (토) | 시즌2 팀빌딩에서 시즌1 발표 | |

⚠️ 6회차만 화요일 (10/5 대체공휴일). 유일한 예외.
...
- **트러블슈팅**: 막혔던 지점은 `issue/weekNN-증상.md`로 정리 → [양식](issue/TEMPLATE.md). 한 명의 에러가 곧 모두의 학습 자료

## 공개 레포 주의

이 레포는 공개입니다. **`passwords.yml`, 개인키(`.pem`), 공인 IP, `/etc/kolla/` 아래 파일은 절대 커밋하지 않습니다.** 스크린샷을 올릴 때도 IP·비밀번호가 보이면 가려주세요. `.gitignore`가 1차로 막지만 최종 책임은 올리는 사람에게 있습니다.
